...
async function fetchProductDetails(productId) {
  try {
    const response = await fetch(
      `https://iti-js-project-backend.vercel.app/api/products/${productId}`
    );
        ...
        <div class="product-page">
          ...
          <div class="product-info"> 
            <nav class="breadcrumb">
              <span style="color:#767676 ">Home </span>/ <span style="color:#767676; ">Accessories</span> / <span>${
                product.name
              }</span> 
            </nav>
            ...
            <div class="actions">
              ...
              ${
                product.quantity > 0
                  ? `<button class="add-to-cart" id="addToCard">Add To Cart</button>`
                  : ""
              }
                        </div>
        <div class="u-desc">
...
<hr>
          <p class="meta" style=" font-size: large;"><span style="color: black;">Categories:</span> ${
...
var token =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NzZjNjBjNjk5MDkzYWZmODRkMTM0NGYiLCJyb2xlIjoidXNlciIsImlhdCI6MTczNTIwMjgxNCwiZXhwIjoxNzM1ODA3NjE0fQ.Xba6hWCTuoMyFkf8yipfJNFj9om4T7vrBJfSooXGItA";
async function getWishList() {
  try {
    const response = await fetch(
      `https://iti-js-project-backend.vercel.app/api/wishlist`,
      {
        headers: {
    ...
    const productExists = wishlist.wishlist.products.find(
...
async function addProductToCart() {
  try {
    const response = await fetch(
      `https://iti-js-project-backend.vercel.app/api/cart`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    const productExistsInCart = cart.cart.items.find(
...
async function getProductsByCategoryID(id) {
  try {
    const response = await fetch(
      `https://iti-js-project-backend.vercel.app/api/categories/${id}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
